...
+ (void)showWithNotificationMessage:(NSString *)message completion:(void (^)(void))completion {
    ...
    if (message) {
        ...
        [APP.notificationWindow addSubview:PUSH_VIEW];
        [APP.notificationWindow makeKeyAndVisible];
        
        APP.notificationWindow.hidden = NO;
        APP.notificationWindow.userInteractionEnabled = YES;
...
+ (void)closeWitCompletion:(void (^)(void))completion {
    ...
    [UIView animateWithDuration:HIDE_ANIM_DUR delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
        ...
        APP.notificationWindow.hidden = YES;
        APP.notificationWindow.userInteractionEnabled = NO;
