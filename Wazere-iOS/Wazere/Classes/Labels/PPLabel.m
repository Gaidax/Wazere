...
- (CFIndex)characterIndexAtPoint:(CGPoint)point {
  ...
  // use label's font and lineBreakMode properties in case the attributedText
  // does not contain such attributes
  [self.attributedText
  ...
  // modify kCTLineBreakByTruncatingTail lineBreakMode to
  ...
  // Offset tap coordinates by textRect origin to make them relative to the
  // origin of frame
  point = CGPointMake(point.x - textRect.origin.x, point.y - textRect.origin.y);
  // Convert tap coordinates (start at top left) to CT coordinates (start at
  // bottom left)
  ...
  CTFramesetterRef framesetter = CTFramesetterCreateWithAttributedString(
      (__bridge CFAttributedStringRef)optimizedAttributedText);

  CGMutablePathRef path = CGPathCreateMutable();
  CGPathAddRect(path, NULL, textRect);

  CTFrameRef frame = CTFramesetterCreateFrame(
      framesetter, CFRangeMake(0, [self.attributedText length]), path, NULL);
  ...
  NSInteger numberOfLines = self.numberOfLines > 0 ? MIN(self.numberOfLines, CFArrayGetCount(lines))
